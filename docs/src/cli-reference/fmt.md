...
If any file requires formatting, the command prints each relative path and exits with status `1`. Clean runs exit with status `0` and no output. Passing a file that is not listed in the configuration results in an error so CI cannot accidentally mutate out-of-scope files.
